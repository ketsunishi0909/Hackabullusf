document.addEventListener('DOMContentLoaded', function() {
    ...
    /* ═══════════════════════════════════════════
       HEADER HIDE ON SCROLL DOWN / SHOW ON UP  [NEW]
    ═══════════════════════════════════════════ */
    ...
    /* ═══════════════════════════════════════════
       FAQ — TABS + ACCORDION  [NEW tabs, improved accordion]
    ═══════════════════════════════════════════ */
    const faqTabs     = document.querySelectorAll('.faq-tab');
    const allFaqItems = document.querySelectorAll('.faq-item');

    // Tab switching — only runs if tabs are present in the HTML
    faqTabs.forEach(tab => {
    ...
    // Accordion — improved with aria + allFaqItems reference
    ...
    /* ═══════════════════════════════════════════
       3-D CAROUSEL  [smoother easing + text slide animation]
    ═══════════════════════════════════════════ */
    const tracks = [
    ...
    /* ═══════════════════════════════════════════
       GALLERY SLIDESHOW  [dot click now resets interval too]
    ═══════════════════════════════════════════ */
    ...
    slides.forEach((_, i) => {
        ...
        // Dot click now also resets the auto-advance timer
    ...
    /* ═══════════════════════════════════════════
       SPONSOR CARDS — scroll-reveal  [improved: unobserve after visible]
    ═══════════════════════════════════════════ */
    ...
    if ('IntersectionObserver' in window) {
        const sponsorObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    entry.target.classList.add('visible');
                    sponsorObserver.unobserve(entry.target); // stop watching once visible
                }
            });
        }, { threshold: 0.15 });
